...
const AddEditBook = ({ setNewBookAdded, bookToUpdate = null }: Props) => {
  ...
  const [author, setAuthor] = useState(
    bookToUpdate ? bookToUpdate?.Author : ""
  );
  const [language, setLanguage] = useState(
    bookToUpdate ? bookToUpdate?.Language : "Bengali"
  );
  const [price, setPrice] = useState(bookToUpdate ? bookToUpdate?.Price : 0);
  const [quantity, setQuantity] = useState(
    bookToUpdate ? bookToUpdate?.Quantity : 0
  );
    ...
    if (name.trim().length > 0 && author.trim().length > 0) {
      ...
      if (bookToUpdate === null) {
        let book: Book = {
          ...
          Quantity: quantity,
          TotalPrice: Number(price.toFixed(2)) * quantity,
          ...
          <Col md={2}>
            <Button variant="primary" type="submit" className="btn-sm">
              {loading ? (
                ...
                `${bookToUpdate !== null ? "Edit" : "Add"} Book`
              )}
            </Button>
            &nbsp;
            {bookToUpdate && (
        ...
        <Row className="mt-3">
          <Col xs={3}>
            <Alert variant="success">
              Book {`${bookToUpdate !== null ? "updated" : "added"}`}
            </Alert>
          </Col>
        </Row>
      )}
